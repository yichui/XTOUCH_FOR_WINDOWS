...
#ifdef MMM
DropShadowEffect scrollbarShadow; // MBadd+++
#endif
...
#ifdef MMM
    const uint32 textHighlightColour   = 0xc0fdbb30;
#else
...
;
    ...
    static const uint32 standardColours[] =
    {
        ...
        TextButton::buttonColourId,                 0XFF444444,
        TextButton::buttonOnColourId,               0xff444444,
        TextButton::textColourOnId,                 0xFFFFAB00,
        TextButton::textColourOffId,                0xFFFFAB00,

        ToggleButton::textColourId,                 0xff000000,

        TextEditor::backgroundColourId,             0xffffffff,
        TextEditor::textColourId,                   0xffffab00,
...
#ifdef MMM
        CaretComponent::caretColourId,              0xffffffff,
#else
        ...
        ScrollBar::backgroundColourId,              0xef000000,
        ScrollBar::thumbColourId,                   0xffffab00,
        //ScrollBar::backgroundColourId,              0xffffffff,
        //ScrollBar::thumbColourId,                  0x701111aa ,
...
#ifdef MMM
        ComboBox::buttonColourId,                   0xff333333, //+++0xffbbbbff,
        ComboBox::outlineColourId,                  0x50000000, //+++standardOutlineColour,
        ComboBox::textColourId,                     0xffFFAB00, //+++0xff000000,
        ComboBox::backgroundColourId,               0x50ffffff, //+++0xffffffff,
        ComboBox::arrowColourId,                    0xfffdbb30, //+++0x99000000,
#else
...
#ifdef MMM
        Slider::trackColourId,                      0xfffdbb30,         //+++0x7fffffff
        Slider::rotarySliderOutlineColourId,        0x66606060, //+++0x66000000,
        Slider::rotarySliderFillColourId,           0xfffdbb30,
#else
        ...
        ProgressBar::foregroundColourId,            0xffffffff,
...
#ifdef MMM
        TabbedComponent::backgroundColourId,        0xff333333,
#else
        TabbedComponent::backgroundColourId,        0xffffffff,
#endif
...
#ifdef MMM
    //scrollbarShadow.setShadowProperties (2.2f, 0.5f, 0, 0); // MBadd+++     setShadowProperties is changed in 2.13    //waittosee
#endif
}
                                        ...
                                        bool isButtonDown)
{
    ...
    const float outlineThickness = button.isEnabled() ? ((isButtonDown || isMouseOverButton) ? 1.6f : 0.8f) : 0.5f;//1.2f : 0.7f) : 0.4f;
    ...
    // set some amber background MBadd+++     
    if(button.getToggleState())
    {
      g.setColour(Colour(0x60fdbb30));
      g.fillRect ( 2, 2, (int)(width - 4), (int)(height - 4));   
    }
/*
      drawGlassLozenge_buttondown (g,
                        indentL,
                        indentT,
                        width - indentL - indentR,
                        height - indentT - indentB,
                        baseColour, outlineThickness, 4.0);
...
*/
    drawGlassLozenge (g,
                      ...
                      width - indentL - indentR,
                      height - indentT - indentB,
                      baseColour, outlineThickness, 4.0,//-1.0f,
...
/*#ifdef MMM
    if(!button.getToggleState()||(button.findColour (TextButton::buttonOnColourId) == Colour(0xffff0000)))
    {
        cl=Colour(0xffa8a8a8);
        g.setColour(cl.withMultipliedAlpha (button.isEnabled() ? 1.0f : 0.3f));
    }
    else
    {
        cl=Colour(0xfffdbb30);
        g.setColour(cl.withMultipliedAlpha (button.isEnabled() ? 1.0f : 0.3f));
    }
#else
     g.setColour (button.findColour (button.getToggleState() ? TextButton::textColourOnId
                               ...
                               const bool isButtonDown)
{
    ...
    drawGlassSphere (g, x, y + (h - boxSize) * 0.5f, boxSize,
        LookAndFeelHelpers::createBaseColour ((ticked?Colour(0xfffdbb30):Colour(0xff333333)).withMultipliedAlpha (isEnabled ? 1.0f : 0.5f),
                                       true,
                                       isMouseOverButton,
                                       isButtonDown),
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++ old 2D look ++++++++++++++++++++++++++++++++                                   
        g.setColour (Colours::black.withMultipliedAlpha ((isButtonDown || isMouseOverButton) ? 1.0f : 0.7f)); 
        g.drawEllipse ( x, y + (h - boxSize) * 0.5f, boxSize, boxSize, 1.6f); 
        if(ticked) {
           g.setColour (Colour(0xfffdbb30).withMultipliedAlpha (isEnabled ? 1.0f : 0.7f));
           g.fillEllipse ( x + boxSize * 0.1, y + boxSize*0.1 + (h - boxSize) * 0.5f, boxSize * 0.8, boxSize * 0.8); 
        }                                   
*/        
#else
    const float boxSize = w * 0.7f;
...
#ifdef MMM
            colour = 0x80ff0000;
            icon.addTriangle (iconRect.getX() + iconRect.getWidth() * 0.5f, (float) iconRect.getY() + 5.0f,
                              (float) iconRect.getRight(), (float) iconRect.getBottom() - 15.0f,
                              (float) iconRect.getX(), (float) iconRect.getBottom() - 15.0f);
#else
            colour = 0x55ff5555;
...
#ifdef MMM
            icon.addEllipse ((float) iconRect.getX() + 5.0f, (float) iconRect.getY() + 5.0f,
                             (float) iconRect.getWidth() - 10.0f, (float) iconRect.getHeight() - 10.0f);
#else
...
{
    //return Font(15.0f);
    return Font ("Arial",15.0f,1);
}

Font LookAndFeel::getAlertWindowFont()
{
    //return Font (12.0f);
	return Font("Arial",12.0f,1);
}
                                 ...
                                 bool /*isMouseDown*/)
{
    ...
    const Colour backgroundColour (scrollbar.findColour (ScrollBar::backgroundColourId ));
    ...
    else
    {
        ...
        //trackColour1 = backgroundColour.overlaidWith (Colour (0x44000000));
        //trackColour2 = backgroundColour.overlaidWith (Colour (0x19000000));
    }
    ...
    g.setGradientFill (ColourGradient (Colours::transparentBlack,gx1, gy1,
                       Colour (0x19000000), gx1, gy2, false));
    g.fillPath (slotPath);

    g.setColour (backgroundColour);
    /*g.setGradientFill (ColourGradient (backgroundColour.brighter (1.97f), gx1,gy1,            
                                          backgroundColour.darker (0.8f), gx2, gy2, false));*/
    g.fillPath (thumbPath);

    /*ColourGradient cg (backgroundColour.darker (0.2f), 0, y,
                           backgroundColour.darker (0.2f), 0, y + height, false);

        cg.addColour (0.03, backgroundColour.withMultipliedAlpha (0.3f));
        cg.addColour (0.4, backgroundColour);
        cg.addColour (0.97, backgroundColour.withMultipliedAlpha (0.3f));
*/
    /*g.setGradientFill (ColourGradient (backgroundColour.withMultipliedAlpha(1.0f), gx1, gy1,
        /*Colours::transparentBlack  backgroundColour.withMultipliedAlpha(0.4f), gx2, gy2, false));*/
        //g.setGradientFill(cg);
    g.setGradientFill (ColourGradient (backgroundColour.brighter (1.97f), x + width * 0.09f,gy1,            
        /* Colours::transparentWhite */backgroundColour.darker (0.8f), gx2, gy2, false));
    g.saveState();
                                             ...
                                             int& idealHeight)
{
    ...
    else
    {
        ...
        idealWidth = font.getStringWidth (text) + idealHeight *1.8f;
    }
}
                                              ...
                                              Slider& slider)
{
    ...
    {
        const float iy = y + height * 0.5f - sliderRadius * 0.25f;        //+++ MBold * 0.5f
        const float ih = sliderRadius * 0.5f;                             //+++ MBnew * 0.5f

        g.setGradientFill (ColourGradient (gradCol1, 0.0f, iy,
                                           gradCol2, 0.0f, iy + ih, false));

        indent.addRoundedRectangle (x - sliderRadius * 0.25f, iy,          //+++ MBold * 0.5f
                                    width + sliderRadius* 0.5f, ih,              //+++ MBnew * 0.5f
                                    2.5f);                                 //+++ MBold * 5.0f
        g.fillPath (indent);
    }
    else
    {
        const float ix = x + width * 0.5f - sliderRadius * 0.25f;         //+++ MBold * 0.5f
        const float iw = sliderRadius * 0.5f;                             //+++ MBnew * 0.5f

        g.setGradientFill (ColourGradient (gradCol1, ix, 0.0f,
                                           gradCol2, ix + iw, 0.0f, false));

        indent.addRoundedRectangle (ix, y - sliderRadius * 0.25f,           //+++ MBold * 0.5f
                                    iw, height + sliderRadius* 0.5f,              //+++ MBnew * 0.5f
                                    2.5f);                                  //+++ MBold * 5.0f
        g.fillPath (indent);
    }
                                         ...
                                         Slider& slider)
{
...
#ifdef MMM
    Colour knobColour (LookAndFeelHelpers::createBaseColour (Colour(0xff5c5c5c),
    ...
    if (style == Slider::LinearHorizontal || style == Slider::LinearVertical)
    {
        ...
        if (style == Slider::LinearVertical)
        {
            ...
            ky = sliderPos;
#ifdef MMM
            /* MBadd new simple sliderknob
            g.setColour(Colour(0xffb2b1b1));
            g.fillRect ( kx-8, ky-12, 16, 24);
            g.setColour(Colours::white);
            g.fillRect ( kx-7, ky-3, 14, 6);
            */
            wx = 16; 
            wy = 16;
#endif
        ...
        else
        {
            ...
            /* MBadd new simple sliderknob
            g.setColour(Colour(0xffb2b1b1));
            g.fillRect ( kx-12, ky-8, 24, 16);
            g.setColour(Colours::white);
            g.fillRect ( kx-3, ky-7, 6, 14);
            */
            wx = 24; 
            wy = 16;
#endif
        ...
        g.fillRect ( (int)(kx - wx*0.5), (int)(ky - wy*0.5), (int)wx, (int)wy);
        drawGlassLozenge(g,
                         kx - wx*0.5f,
                         ky - wy*0.5f,
                         wx,
                         wy,
                         knobColour, outlineThickness, 2.0,//-1.0f,
                         false,
                         false,
                         false,
                         false);   

        // draw some box (white)... MBadd
        //    g.setColour(Colour(0xffffffff));
        //    g.fillRect ( (int)(kx - wx*0.25), (int)(ky - 4), (int)(wx*0.5), 6);                      
        // draw some lines... MBadd
        g.setColour(Colour(0xffffffff));
        g.drawLine ( kx - wx*0.35f, ky - 3, kx + wx*0.4f, ky - 3);
        g.drawLine ( kx - wx*0.35f, ky,     kx + wx*0.4f, ky);
        g.drawLine ( kx - wx*0.35f, ky + 3, kx + wx*0.4f, ky + 3);
#else
        drawGlassSphere (g,
                         kx - sliderRadius,
                                    ...
                                    Slider& slider)
{
    ...
    const bool isMouseOver = slider.isMouseOverOrDragging() && slider.isEnabled();
    const float thickness = 0.8f; //++0.7f;

    if (false) //(thickness > 0)
    {
...
//    g.strokePath (outlineArc, PathStrokeType (slider.isEnabled() ? (isMouseOver ? 2.0f : 1.2f) : 0.3f));
...

    {
        ...
        filledArc.addPieSegment (rx, ry, rw, rw,
                                rotaryStartAngle,           //original Marting doesn't use this line ???
//                                slider.rotaryNull,        //original Marting uses this line ???, if to use this line, there is build fail
                                angle,
                                thickness);
...
/*
    if (slider.isEnabled())
        g.setColour (Colour (0xff5c5c5c).withAlpha (isMouseOver ? 1.0f : 0.7f));
    else
        g.setColour (Colour (0x80808080));

    Path p;
    p.addEllipse (-0.4f * rw, -0.4f * rw, rw * 0.8f, rw * 0.8f);
//    PathStrokeType (rw * 0.1f).createStrokedPath (p, p);                // to draw cycle only
//    p.addLineSegment (0.0f, 0.0f, 0.0f, -rw * 0.4, rw * 0.2f);          // to draw cycle only
    g.fillPath (p, AffineTransform::rotation (angle).translated (centreX, centreY));
*/
   
    drawGlassSphere (g, centreX -0.4f * rw, centreY -0.4f * rw, rw * 0.8f,
                     LookAndFeelHelpers::createBaseColour (Colour(0xff333333).withMultipliedAlpha (1.0f),
                                       true,
                                       isMouseOver,
                                       false),
                                       (isMouseOver ? 1.0f : 0.6f));
    ...
    Path pp;
    g.setColour (Colour (0xffffffff)); //0xffffffff));
    pp.addLineSegment (Line<float> (0.0f, 0.0f, 0.0f, -rw * 0.34f), rw * 0.1f);
    g.fillPath (pp, AffineTransform::rotation (angle).translated (centreX, centreY));
#else
    const float radius = jmin (width / 2, height / 2) - 2.0f;
    const float centreX = x + width * 0.5f;
    const float centreY = y + height * 0.5f;
...
#ifdef MMM
    l->setColour (TextEditor::backgroundColourId, Colours::transparentBlack);//+++ ...ColourId,
//                  slider.findColour (Slider::textBoxBackgroundColourId)
...
#ifdef MMM
    g.setColour(Colour(0xffa8a8a8).withMultipliedAlpha (alpha)); // +++ set color +++
#else
...
#ifdef MMM
    return jlimit (tabDepth * 2, tabDepth * 8, width) + 24;
#else
...
#ifdef MMM
    const float indent = 0.0f; //(float) getTabButtonOverlap ((int) depth);
#else
...
#ifdef MMM
    g.setColour (isFrontTab ? Colours::black //+++preferredColour
        : Colour(0xff595959));
#else
...
void LookAndFeel::drawTabButtonText (TabBarButton& button, Graphics& g, bool isMouseOver, bool isMouseDown)
{
    ...
    Font font ("Arial",depth * /*0.59f*/0.54,1);
    //Font font ("Arial",jmin (15.0f, getHeight() * 0.6f) ,1 /* +++ -> set to 1 if you want bold! +++ */);
    font.setUnderline (button.hasKeyboardFocus (false));

#ifdef MMM
    if(button.isFrontTab()){  // +++ set bold and high for active one +++
       font.setBold(true); 
       //button.y-=2;
       //y-=2;
    }
    ...
    const float alpha = button.isEnabled() ? ((isMouseOver || isMouseDown) ? 1.0f : 0.7f) : 0.3f;
    ...
    {
        cl = Colour(0xfffdbb30);
        g.setColour (cl.withMultipliedAlpha (alpha));
    }else
    {
        cl = Colour(0xffe0e0e0);
        g.setColour (cl.withMultipliedAlpha (alpha));
    }
        //g.setColour (Colour(0xfffdbb30));
#else
...
void LookAndFeel::drawTableHeaderBackground (Graphics& g, TableHeaderComponent& header)
{
    ...
    g.setGradientFill (ColourGradient (Colour (0xff5c5c5c), 0.0f, h * 0.5f,
                                       Colour (0xff333333), 0.0f, h - 1.0f,
                                       false));
    g.fillRect (0, 0, w, h);
                                            ...
                                            const Path& path, Image& cachedImage)
{
    ...
    g.setColour (Colour::greyLevel (0.38f).withAlpha (0.9f));
    g.fillPath (path);

    g.setColour (/*Colours::white*/Colour(0xffffab00).withAlpha (0.8f));
...
//        cg.addColour (0.4, Colours::white.overlaidWith (colour));
        ...
        cg.addColour (0.5, Colours::white.overlaidWith (colour));
        cg.addColour (0.95, colour);
...
//                                       Colours::transparentWhite, 0, y + diameter * 0.3f, false));
    g.setGradientFill (ColourGradient (colour.brighter (1.0f), 0, y + diameter * 0.06f,
                                       Colours::transparentWhite, 0, y + diameter * 0.5f, false));
...
#ifdef MMM
        g.setGradientFill (ColourGradient (colour.brighter (2.0f), 0, y + height * 0.05f,            
                                           Colours::transparentWhite, 0, y + height*0.5f, false));
#else
...
//==============================================================================  ++++++++++++++++ MBadd +++++++++++++++
void LookAndFeel::drawGlassLozenge_buttondown (Graphics& g,
                                    const float x, const float y,
                                    const float width, const float height,
                                    const Colour& colour,
                                    const float outlineThickness,
                                    const float cornerSize) throw()
{
    ...
    Path outline;
    createRoundedPath (outline, x, y, width, height, cs, true, true, true, true); 
    {
        ColourGradient cg (colour.darker (0.2f), 0, y,
                           colour.darker (0.2f), 0, y + height, false);
    ...
    {
        const float leftIndent = cs * 0.4f;
        const float rightIndent = cs * 0.4f;
        ...
        g.setGradientFill (ColourGradient (Colours::transparentBlack, 0, y + height * 0.5f,
                                           colour.brighter (10.5f), 0, y + height, false));
        g.fillPath (highlight);
    }
