...
#ifdef MMM
    b->setColour (TextButton::buttonColourId, Colour (0xff444444));
    b->setColour (TextButton::buttonOnColourId, Colour (0xff444444));
    b->setColour (TextButton::textColourOnId, Colour (0xffffab00));
    b->setColour (TextButton::textColourOffId, Colour (0xffa8a8a8));

    b->setToggleState(true, dontSendNotification); //this is to have the text color to be in yellow
#endif
                                 ...
                                 const bool isPasswordBox)
{
    ...
    //if(String(onScreenLabel).compare("")==0)
...
public:
    ...
    void updateLayout (const int width)
    {
        ...
        //text.createLayoutWithBalancedLineLengths (s, width - 13.0f);
...
void AlertWindow::updateLayout (const bool onlyIncreaseSize)
{
    ...
    attributedText.append (getName(), font.withHeight (font.getHeight() * /*1.1f*/1.0f).boldened());
